...
def test_check_old_running_jobs_with_resubmitted(
    ...
    with config().db.session() as sess:
        base_job: SlurmJobDB
        base_job, = sess.exec(select(SlurmJobDB).where(SlurmJobDB.job_state == SlurmState.RUNNING)).all()
        job = SlurmJobDB.model_validate(base_job)
        job.submit_time += timedelta(hours=1)
        job.start_time += timedelta(hours=1)
        ...
        new_jobs = sess.exec(select(SlurmJobDB).where(SlurmJobDB.job_state == SlurmState.RUNNING)).all()
