...
def test_invalid_time_unit(cli_main, caplog):
    with gifnoc.overlay(
        ...
        assert bool(
            re.search(
                r"WARNING +.+\.py:[0-9]+ +Invalid time unit \(must be > 0\) for cluster usage checking: 0:00:00",
                caplog.text,
            )
        )
        assert bool(
...
def test_invalid_nb_stddev(cli_main, caplog):
    with gifnoc.overlay(
        ...
        assert bool(
            re.search(
                r"WARNING +.+\.py:[0-9]+ +Invalid nb_stddev \(must be >= 0\) for cluster usage checking: -21",
                caplog.text,
            )
        )
        assert bool(
...
def test_empty_database(cli_main, caplog):
    with gifnoc.overlay(
        ...
        print(caplog.text)
        assert bool(
            re.search(r"WARNING +.+\.py:[0-9]+ +No jobs in database", caplog.text)
        )
        assert bool(
            re.search(
                r"ERROR +.+\.py:[0-9]+ +\[cluster_scraping] FAILURE: cluster_scraping",
