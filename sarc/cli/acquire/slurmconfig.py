...
class SlurmConfigParser:
    ...
    def _parse_gpu_to_billing(
        ...
        # Use to check that inferred billing for a GPU is the same across partitions.
...
class Partition:
    ...
    def get_node_gpus(self, node_to_gpu: Dict[str, str]) -> List[str]:
        return sorted(
            {
                gres
    ...
    def get_gpus_and_partition_billings(self, node_to_gpu: Dict[str, str]):
        local_gres = self.get_node_gpus(node_to_gpu)
